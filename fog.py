...
def gen_fog(img_path: Path, depth_path: Path, output_path: Path) -> None:
    ...
    if const.FT == 0:  # only haze: const.FT should be set to 0
        ...
        I[:, :, 0] = img[:, :, 0] * np.exp(-ECA*distance_through_haze-ECM*distance_through_haze_free)
        I[:, :, 1] = img[:, :, 1] * np.exp(-ECA*distance_through_haze-ECM*distance_through_haze_free)
        I[:, :, 2] = img[:, :, 2] * np.exp(-ECA*distance_through_haze-ECM*distance_through_haze_free)
